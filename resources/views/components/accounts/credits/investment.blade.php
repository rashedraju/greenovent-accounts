        ...
        <tbody>
            @foreach ($credits as $credit)
                <tr class="fw-bold">
                    ...
                    <td class="px-2 py-5">{{ $credit->company_name }}</a>
                ...
                <x-drawer btnId="edit_investment_drawer_btn_{{ $loop->iteration }}"
                    drawerId="edit_investment_drawer_{{ $loop->iteration }}" title="Edit laon credit record">
                    <x-forms.accounts.credits.edit-investment :credit="$credit" :employees="$employees" :transactionTypes="$transactionTypes" />
                </x-drawer>

                <x-drawer btnId="delete_investment_drawer_btn_{{ $loop->iteration }}"
                    drawerId="delete_loan_drawer_{{ $loop->iteration }}" title="Delete investment credit Record">
                    <form method="post" action="{{ route('accounts.credits.investment.delete', $credit) }}">
                        @csrf
                        @method('delete')
