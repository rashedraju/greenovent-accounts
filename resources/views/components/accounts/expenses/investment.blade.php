        ...
        <tbody>
            @foreach ($expenses as $expense)
                <tr class="fw-bold">
                    ...
                    <td class="px-2 py-5">{{ $expense->company_name }}</a></td>
                ...
                @if ($editable)
                    <x-drawer btnId="edit_investment_drawer_btn_{{ $loop->iteration }}"
                        ...
                        <x-forms.accounts.expenses.edit-investment :expense="$expense" :employees="$employees"
                            :transactionTypes="$transactionTypes" />
                    </x-drawer>

                    <x-drawer btnId="delete_investment_drawer_btn_{{ $loop->iteration }}"
                        drawerId="delete_loan_drawer_{{ $loop->iteration }}"
                        title="Delete investment expense Record">
