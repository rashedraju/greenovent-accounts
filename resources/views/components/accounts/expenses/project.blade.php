@props(['heads', 'projects', 'expenses', 'employees', 'transactionTypes', 'editable' => true])
        ...
        <tbody>
            @foreach ($expenses as $expense)
                <tr class="fw-bold">
                    ...
                    <td class="px-2 py-5">{{ $expense->project->name }}</a></td>
                ...
                @if ($editable)
                    <x-drawer btnId="edit_project_drawer_btn_{{ $loop->iteration }}"
                        drawerId="edit_project_expense_drawer_{{ $loop->iteration }}"
                        title="Edit project expense record">
                    ...
                    <x-drawer btnId="delete_project_drawer_btn_{{ $loop->iteration }}"
                        drawerId="delete_project_drawer_{{ $loop->iteration }}"
                        title="Delete project expense Record">
