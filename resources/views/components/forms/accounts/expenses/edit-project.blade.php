@props(['startDate', 'endDate', 'heads', 'projects', 'expense', 'employees', 'transactionTypes'])
<form action="{{ route('accounts.expenses.project.update', $expense) }}" method="post">
    @csrf
    @method('put')
    ...
    <select class="form-select" name="project_id">
        ...
        @foreach ($projects as $projectId => $projectName)
            <option value="{{ $projectId }}" {{ $expense->project->id == $projectId ? 'selected' : '' }}>
                {{ $projectName }}</option>
        @endforeach
    </select>
